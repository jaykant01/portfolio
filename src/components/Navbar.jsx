    ...
    return (<nav className="fixed top-0 left-0 w-full z-50">
            ...
            md:my-2  md:rounded-xl ">
                ...
                <div className="hidden md:flex space-x-8">
                    {LINKS.map((link, index) => (<a href={link.href} key={index}
                ...
                <div className="md:hidden">
                    <button onClick={() => setMenuOpen(!menuOpen)}
                        ...
                        {menuOpen ? (<RiCloseFill className="w-6 h-6"/>) : (<RiMenu3Fill className="w-6 h-6 mr-4"/>)}
                    </button>
                </div>
            </div>
            {menuOpen && (<div
                    className="md:hidden p-2 bg-stone-950/30 backdrop-blur rounded-xl flex flex-col space-y-4 max-w-6xl mx-auto">
                    {LINKS.map((link, index) => (<a href={link.href} key={index}
                                                    className="text-white hover:text-stone-400 transition duration-300"
